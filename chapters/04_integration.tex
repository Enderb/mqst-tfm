...
delay $\tau$ (the round-trip propagation delay in the loopback cable), the phase
offset scales linearly with frequency:
\begin{equation}
  \phi(f) = \phi_0 - 360^\circ \cdot \tau \cdot f,
\label{eq:phase_model}
\end{equation}
where $\phi_0$ is a constant offset and $\tau \approx \SI{251}{\micro\second}$
in the as-assembled loopback (dominated by the cable length, not the on-chip
delay).
