...
front end: sweeping the same loopback path with and without the readout low-pass
filter (a Mini-Circuits VLF-7200+) placed its $-3$\,dB edge at
${\approx}\SI{8.4}{\giga\hertz}$, against the \SI{8.15}{\giga\hertz} nominal
specification and clear of the swept band.
The single \texttt{axis\_signal\_gen\_v6} path above is the one used for
...
sessions, so results cannot be compared across power cycles.
