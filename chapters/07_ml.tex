...
Each single-shot readout produces an averaged $(I, Q)$ pair; repeated preparation
and measurement builds up two clouds in the complex plane, one per state, whose
centroids and spreads depend on readout power, resonator parameters, and
integration time.  As set
...
to the readout window and $T_1$, a few hundred nanoseconds at most),
resource usage (it must fit in the available DSP slices and block RAM without
displacing the readout and tProcessor logic), and trainability (training on a
...
slices on the ZCU216.  The sections below evaluate these classifiers offline on
existing experimental data; FPGA deployment is discussed in
Section~\ref{subsec:deployment_path}, and prior neural-network discrimination
work, including multiplexed readout, in Lienhard et al.~\cite{Lienhard2022}.

\subsection{Experimental Data}
\label{subsec:ssro_data}

The dataset used throughout this chapter is \texttt{SSRO.h5}, collected on a
superconducting qubit device at Qilimanjaro and retrieved from a database of past
experiments.  It contains 2000 single-shot IQ
...
data, which limits how precisely the synthetic traces of Part~III can be
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.5\columnwidth]{code/figures/fig1_iq_scatter}
  \caption{IQ scatter of the \texttt{SSRO.h5} dataset (2000 shots per state;
    blue $\ket{0}$, orange $\ket{1}$; filled circles mark the centroids).  The
    clouds are well-separated and approximately Gaussian, the $\ket{1}$ ellipse
    visibly wider (the $\approx\!1.7\times$ covariance asymmetry of
    Table~\ref{tab:iq_stats}), and both are tilted by the residual IQ rotation
    noted in the text.}
...
The $\ket{0}$ accuracy (94.0\%) exceeds the $\ket{1}$ accuracy (87.3\%): a
diagnostic signature of $T_1$ relaxation during the readout window, which makes
some $\ket{1}$ shots integrate into the $\ket{0}$ blob.
...
Demonstrating this directly requires raw ADC traces from a qubit device, and none
were available for this thesis: the qubit
...
mid-shot relaxation step.  This lets the full pipeline, from raw
trace to trained CNN, be developed and validated now, ready to run on
real traces as soon as a chip is available.

\subsubsection{Synthetic Trace Model}

Each shot is modelled as a noisy cavity response: the intracavity field rings up
toward its steady state at a rate set by the resonator linewidth
...
shots.  This is deliberately larger than on the real device (measured $\ket{1}$
accuracy 87.3\%): the
synthetic set over-represents mid-readout relaxation as a stress test of the
recovery the CNN is meant to perform.  Recalibrating $T_1$ to the real error rate
is left to future work (Section~\ref{sec:conclusions}).  The simulation
...
into training and test sets.  The loss converges within about 40
...
CNN's gain comes from its nonlinearity, not merely from seeing more input.  That
it lands marginally \emph{below} LDA despite the richer input is expected: with
i.i.d.\ per-sample noise and no relaxation the window mean is a sufficient
...
$t^*$: late relaxers are classified with near-certainty, while early relaxers are
correctly flagged as ambiguous
...
The qualitative conclusion is robust: a CNN operating on raw
traces has access to temporal information the rectangular integrator discards, an
advantage that persists regardless of the exact $T_1$.  Retraining on
real raw traces once collected is the natural
...
outlines the steps required to close the loop
...
models~\cite{Fahim2021}, is expected to decide within
\SIrange{100}{200}{\nano\second} at \SI{250}{\mega\hertz}, comfortably inside the
latency budget of Section~\ref{subsec:classifiers}.
...
\begin{enumerate}
  \item \textbf{Collect real raw traces}: connect the ZCU216 to a qubit chip and
        ...
        $(2, N_\text{shots}, T, 2)$), with \texttt{readout\_length} $\geq 3/\kappa$
        to capture the full cavity ringdown.
  \item \textbf{Retrain and validate} on real traces, recalibrating the
...
From the model's 953 parameters and typical \texttt{hls4ml} resource scaling,
the CNN IP block is expected to need on the order
of \num{10000} LUTs and tens of DSP slices, a small fraction of the ZCU216's
fabric, though this must be confirmed by synthesis.
