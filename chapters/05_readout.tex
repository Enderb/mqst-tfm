...
separation of \SI{2.53}{ADU} and a projected SNR of 2.46.  The single-shot
assignment fidelity extracted from these clusters is 0.906 (the LDA baseline of
Section~\ref{subsec:integrated_iq}), within the range of reported high-fidelity
single-shot dispersive readouts~\cite{Walter2017}.
...
round-trips.  The readout down-conversion frequency must track the drive to keep the
demodulated signal at baseband, though, and it is set per program rather than stepped
in this loop; for spectroscopy, where the two must move together, this sweep alone is
insufficient.
