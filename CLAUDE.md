...
- tProcessor v2 clocks (from the firmware soccfg printout stored in `code/qick/frequency_sweep_tprocv2.ipynb` outputs — the source of truth): **200 MHz core execution clock** (`qick_processor` rev 21; 1 core cycle = 5 ns) and **614.4 MHz dispatcher timing clock**. The old "384 MHz fabric clock → 2.6 ns/cycle" was a v1-era error, corrected everywhere in the FABLE round-2 pass (ch3 bullet, §6.1, §6.4, Table 3). Decimated readout rate = fs/8 = 307.2 MS/s at 2.4576 GS/s (also confirmed by soccfg).
- Active reset digital latency: `condj` (~5 ns, 1 core cycle) + threshold (~10 ns, a few core cycles); thesis says "tens of ns", "<100 ns"
- VLF-7200+ readout LPF (Setup 1, gen 0 → RO 0; Setup 2 = gen 3 → RO 6, no filter): measured −3 dB edge 8444.4 MHz vs 8150 MHz nominal, from the same-path with/without-filter sweep in `code/qick/frequency_sweep_v2_samepath.ipynb`; quoted in §4.2 as ≈8.4 GHz. Do NOT quote the printed "11.10 dB passband insertion loss" — it includes unnormalised connector/path losses (the notebook itself normalises it away).
...
**FABLE feedback round 2 (July 6 2026, same branch)** — `FABLE_FEEDBACK_2.md` confirmed round 1 resolved and listed small items, all applied: Fig. 5 caption no longer claims the curve "remains" above the LDA baseline (it dips to it early); Fig. 4 (synthetic traces) regenerated with **ADU** axis labels (notebook cell updated + seeded rng-state replication, so the same 30 shots are drawn); §4.2 dangler closed with the measured VLF-7200+ result (−3 dB at ≈8.4 GHz vs 8.15 GHz nominal); §5.2 "typical value" claim now cites Krantz2019; logreg-below-LDA explained (window mean is a sufficient statistic under i.i.d. noise → extra weights only add estimation variance); and the reviewer's "spot-check 384 MHz / 32 registers" item surfaced a real error — the tProc clocks were corrected from the soccfg printout (see Key constants above; the 32×64-bit-registers-per-page description is now explicitly attributed to the v1 design). Round-2 additions paid for by trims in ch7/ch8 (no content lost, mostly dedup and tightening); main text re-verified **exactly 20 pages**, build clean. NOT done (no data in repo): fitted τ value + cross-reboot repeatability for §4.3 (feedback Q12 nice-to-have) — needs the author's phase-cal numbers.
